function QLearningWithOptionsStochasticDomainFunc(DiffusionOptionsValueFunc,LaplacianEignOptions,RandomOptionsValueFunc,StochasticRatio,StateMat,StatesNoTransitionIndx,FourRoomFlag,HardMazeDomainFlag,RingRoomFlag,KStates,KDim1,KDim2)
%% The code implements Q learning according to  Watking & Dayan 1992
% For stochastic domain
%%
% close all
rng(3) 
%% Numeric Params
...
for mc=1:KMonteCarlo
    ...
    StateVisitationArrTmp           = zeros(KConfig,KStates); % array that computes the # visitation during learning
    for gs=1:length(GoalStateDomain)
        ...
        for confg=1:KConfig
            if confg==1
                OptionsTypeFlag         = 1;    % 2: Random Options 1: Diffusion options 0: Laplacian eigen options
            ...
            for e=1:KEpisodes
                ...
                for step = 1:KStepsInEpisode
                    ...
                    if IsOneMaximum   
                        ...
                        else % The action down is not possible
                            ActionChosen                    =  MaxQFuncIndxCurState;
                        end
                        ...
                        if IsOptionChosen*(~OnlyPrimitiveAction)        % option
                            ...
                            while ~any(CurState==OptionGoalState) 
                                ...
                                if OptionToDoValueFunc(NextState)<=OptionToDoValueFunc(CurState)  
                                    break;
                                end
                                %-----updating Q function
                                QFunc(CurState,ActionChosen)            = (1-Alpha)*QFunc(CurState,ActionChosen) + Alpha*( Reward(CurState)+Gamma*max(QFunc(NextState,:)) );
                ...
                for t=1:KTrials
                    ...
                    %----------------Adding stochasticity----------------------
                    if any(AllowedActionsCell{CurStateTest}==4)  
                        if randi(StochasticRatio)==StochasticRatio
                            ActionChosenTest = 4;
                            NextStateTest = Act(ActionChosenTest,CurStateTest,KDim1);
                        end
